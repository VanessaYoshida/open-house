...
let onSuccess = (result) => {
  ...
  window.marker = new H.map.Marker({ lat: locations[0].location.displayPosition.latitude,
    lng: locations[0].location.displayPosition.longitude }, { icon: icon });
...
let map = new H.Map(document.getElementById('map'),
  defaultLayers.normal.map, {
    center: {lat: localStorage.getItem('latitude'),
...
let icon = new H.map.Icon('./assets/img/home.png');
window.marker = new H.map.Marker({ lat: localStorage.getItem('latitude'),
  lng: localStorage.getItem('longitude') }, { icon: icon });
map.addObject(marker);
let bubble;
